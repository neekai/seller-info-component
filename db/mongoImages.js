...
const imageDataFile = __dirname + '/../data/mongoImages.txt';
  ...
  function write(){
    ...
    do {
      ...
      if(i === 0){
        ...
        wstream.write(indexGenerator() + ',' + randomImage + ',' + productID + '\n', (err) => {
          if(err){
        ...
        const randomImage = imageURL[index];
        const productID = Math.ceil(Math.random()*10000000);
        ok = wstream.write(indexGenerator() + ',' + randomImage + ',' + productID + '\n', (err) => {
        if(err){
          console.log("There was an err writing the file", err);
          } 
       })
